...
public class NotificationsListView extends AdapterView<ListAdapter>  {
	...
	public NotificationsListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initView();
	...
	private DataSetObserver mDataObserver = new DataSetObserver() {
		...
		public void onChanged() {
			synchronized(NotificationsListView.this){
				mDataChanged = true;
			}
			invalidate();
			requestLayout();
	...
	protected boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
		...
		synchronized(NotificationsListView.this){
			...
			//	mScroller.fling(mNextX, 0, (int)-velocityX, 0, mActiveFeature*featureWidth, mActiveFeature*featureWidth, 0, 0);
				...
				comm.setPageMarkersNotifications(mActiveFeature);
				}
				else {
	...
	private OnGestureListener mOnGesture = new GestureDetector.SimpleOnGestureListener() {
		...
		public boolean onDown(MotionEvent e) {
			return NotificationsListView.this.onDown(e);
		}

		@Override
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
				float velocityY) {
			return NotificationsListView.this.onFling(e1, e2, velocityX, velocityY);
		}
			...
			for(int i=0;i<getChildCount();i++){
				...
				if (isEventWithinView(e, child)) {
					if(mOnItemClicked != null){
						mOnItemClicked.onItemClick(NotificationsListView.this, child, mLeftViewIndex + 1 + i, mAdapter.getItemId( mLeftViewIndex + 1 + i ));
					}
					if(mOnItemSelected != null){
						mOnItemSelected.onItemSelected(NotificationsListView.this, child, mLeftViewIndex + 1 + i, mAdapter.getItemId( mLeftViewIndex + 1 + i ));
					}
					break;
				}
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (isEventWithinView(e, child)) {
					if (mOnItemLongClicked != null) {
						mOnItemLongClicked.onItemLongClick(NotificationsListView.this, child, mLeftViewIndex + 1 + i, mAdapter.getItemId(mLeftViewIndex + 1 + i));
					}
					break;
				}
